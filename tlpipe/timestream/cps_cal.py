...
class PsCal(timestream_task.TimestreamTask):
    ...
    def process(self, ts):
        ...
        if save_src_vis or subtract_src or apply_gain or save_gain:
            ...
            if ts.is_disk:
                obsd_sources = ts.parent['transitsource'].attrs['scrname']
                srcdict = {'cas':'CassiopeiaA'}
                src_index = 0.1
                for index, src_short, src_long in enumerate(srcdict.items()):
                ...
                obs_transit_time = ts.parent['transitsource'][src_index][0]
            ...
            next_transit = aa.next_transit(s) if not ts.is_disk else ephem.date(datetime.utcfromtimestamp(obs_transit_time))
            ...
            while(transit_time <= ts['jul_date'][-1]):
                ...
                transit_time = a.phs.ephem2juldate(aa.next_transit(s) if not ts.is_disk else ephem.date(datetime.utcfromtimestamp(obs_transit_time))) # Julian date
                cnt += 1
            ...
            if apply_gain or save_gain:
                ...
                if path('feedplotG%d.npy'%savenum):
                    savenum += 1
                else:
